...
async function createIndexes(): Promise<void> {
  try {
    ...
    // Note: ExamSubmission now only stores completed submissions
    ...
    await ExamSubmissionModel.collection.createIndex(
      ...
      { name: 'userId_submittedAt_idx' }
    );
    console.log('✅ Created index: userId_submittedAt_idx (user submission history)');
    
    await ExamSubmissionModel.collection.createIndex(
      { marksAchieved: -1 },
      { name: 'marksAchieved_idx' }
    );
    console.log('✅ Created index: marksAchieved_idx (leaderboards and score filtering)');
    
    await ExamSubmissionModel.collection.createIndex(
      { examId: 1, marksAchieved: -1 },
      { name: 'examId_marksAchieved_idx' }
    );
    console.log('✅ Created index: examId_marksAchieved_idx (exam-specific rankings)');
    
    await ExamSubmissionModel.collection.createIndex(
      { autoSubmitted: 1, submittedAt: -1 },
      { name: 'autoSubmitted_submittedAt_idx' }
    );
    console.log('✅ Created index: autoSubmitted_submittedAt_idx (track auto-submitted exams)');
