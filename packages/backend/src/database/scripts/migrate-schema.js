 ...
 * 
 * Usage:
 *   node migrate-schema.js [--dry-run] [--batch-size=100]
 * 
 * Options:
 ...
 */
async function migrateQuestionsToPrompts() {
  ...
  const questionToPromptMap = new Map(); // Maps old question ID to new prompt ID
  let processed = 0;
  let migrated = 0;
    ...
    for (const question of questions) {
      try {
        ...
        const promptData = {
          ...
          generateVia: 'user', // Existing questions are user-created
 ...
 */
async function createExamQuestions(questionToPromptMap) {
  ...
  const examQuestionMap = new Map(); // Maps exam ID to array of ExamQuestion IDs
  let processed = 0;
      ...
      try {
        ...
        for (const examQuestion of exam.questions) {
          ...
          const examQuestionData = {
            ...
            answerType: 'mcq', // Default, adjust based on your data
 ...
 */
async function updateExams(examQuestionMap) {
  ...
  for (const [examId, examQuestionIds] of examQuestionMap) {
    try {
      ...
      console.error(`  ❌ Error updating exam ${examId}:`, error.message);
    }
  }
 ...
 */
async function updateExamSubmissions() {
          ...
          await ExamSubmissionModel.updateOne(
            { _id: submission._id },
            {
              $set: {
                ...
                status: submission.evaluated ? 'EVALUATED' : 
                       submission.submittedAt ? 'SUBMITTED' : 'IN_PROGRESS',
...
async function verifyMigration() {
  ...
  const orphanedExamQuestions = await ExamQuestionModel.countDocuments({
    promptId: { $nin: (await PromptModel.distinct('_id')).map(id => id.toString()) }
  });
...
async function runMigration() {
  try {
    ...
    if (isDryRun) {
      ...
      console.log('1. Create database indexes by running: node create-indexes.js');
